...
import nine.jasper.dynamic.StyleStatics;
...
public class CustomTextSubtotalReport {
	...
	private void build() {
		...
		try {
			report()
			  .setTemplate(StyleStatics.reportTemplate.setLanguage(Language.JAVA))
			  .variables(
			  ...
			  //.summary(
			  	//summarySbt)
			  .title(StyleStatics.createTitleComponent("CustomTextSubtotal"))
			  .pageFooter(StyleStatics.footerComponent)
	...
	private class CustomTextSubtotal extends AbstractComplexExpression<String> {
		...
		public CustomTextSubtotal(VariableBuilder<Integer> quantitySum, VariableBuilder<BigDecimal> priceSum) {
			...
			addExpression(field("country", type.stringType()));
		}
		...
		public String evaluate(List<?> values, ReportParameters reportParameters) {
			...
			return group + " sum(quantity) = " + type.integerType().valueToString(quantitySum, reportParameters) + ", " +
